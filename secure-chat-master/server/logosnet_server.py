...
def check_cert(user, cert, usernames):
    '''
    Checks if username is available and a matching certificate exists, returns string code with status of username
    '''
    user = user + "\n"
    path_to_key = "ca-key-public.pem"
    try:
        pub_key = serialization.load_pem_public_key(open(path_to_key,'rb').read())
        pub_key.verify(cert.encode(), user.encode(), padding.PKCS1v15(), hashes.SHA256())
    except:
        return "USERNAME-INVALID"
    finally:
        return is_username(user, usernames)
	 ...
	 ###
	         ...
	         #Username exists for this client, this is a message
                    if s in usernames:
                        ...
                        msg = "> " + usernames[s] + ": " + msg
                        if pvt_user and (msg_id.startswith("DH") or msg_id == "PVT-MSG"):
                        #dh key exchange desired
                        #elif msg_id.startswith("DH") or msg_id == "PVT-MSG":
                            tgt_user = msg.split(" ")[2][1:]
                            print(tgt_user, "msg", msg, msg_id)
                            for k,v in usernames.items():
                                if v == tgt_user:
                                    LNP.send(k, msg[msg.index(':')+2:], msg_id)
	         ...
	         #no username yet, this message is a username
                        ...
                        cert_msg = msg.split("|")
                        username_status = check_cert(cert_msg[0], cert_msg[1], usernames)
                        LNP.send(s, '', username_status)

                        if username_status == "USERNAME-ACCEPT":
                            usernames[s] = cert_msg[0]
                            del user_connect_time[s]
